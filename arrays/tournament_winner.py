...
def tournamentWinner(competitions, results):
    ...
    for idx, match in enumerate(competitions):
        winner = match[0] if results[idx] == 1 else match[1]
        if winner not in win_count_map:
            win_count_map[winner] = 3
        win_count_map[winner] += 3
        tournament_winner = (
            winner
            if win_count_map[winner] > win_count_map.get(tournament_winner, 0)
            else tournament_winner
        )
